...
def test_add_reference_creates_file(skills_root):
    ok = stu.add_reference("reply/foo", "1.0", "2026-05-10T00:00:00+00:00")
    assert ok is True
    path = skills_root / "reply" / "foo" / "thumbs_up.json"
    assert path.is_file()
...
def test_add_reference_dedupes(skills_root):
    stu.add_reference("reply/foo", "1.0", "a")
    stu.add_reference("reply/foo", "1.0", "a")
    stu.add_reference("reply/foo", "1.0", "b")
    refs = stu.recent_references("reply/foo")
    assert refs == [
...
def test_add_reference_invalid_skill_id(skills_root):
    ...
    assert stu.add_reference("not_a_skill_id", "1.0", "a") is False


def test_add_reference_missing_values(skills_root):
    assert stu.add_reference("reply/foo", "", "a") is False
    assert stu.add_reference("reply/foo", "1.0", "") is False


def test_caps_at_max_kept(skills_root):
    for i in range(stu._MAX_KEPT + 10):
        stu.add_reference("reply/foo", "1.0", f"a{i}")
    refs = stu.recent_references("reply/foo", limit=stu._MAX_KEPT + 50)
...
def test_recent_references_missing_file(skills_root):
    assert stu.recent_references("reply/foo") == []


def test_recent_references_limit_zero(skills_root):
    stu.add_reference("reply/foo", "1.0", "a")
    assert stu.recent_references("reply/foo", limit=0) == []
