...
- **Draft replies** — via message shortcut, @mention, or `/copilot`; uses channel RAG + cross-channel RAG + skills (progressive disclosure on each of those entry points)
...
- **Follow-ups (action items)** — [`skill_examples/follow_up/SKILL.md`](../skill_examples/follow_up/SKILL.md); install under `~/.open_slack_copilot/skills/follow_up/`; uses `schedule_prompt`, `list_usergroup_members`, `send_dm_as_app`
...
- **Saved skills** — reusable behaviors in `~/.open_slack_copilot/skills/<name>/SKILL.md`; progressive disclosure selects skills for `/copilot`, shortcut, and @mention drafts
...
- Watch channels & match skills (M4)
